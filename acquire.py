...
def get_news_articles_data(refresh=False):
    ...
    if not os.path.isfile('news_articles.csv') or refresh:
        ...
        # Use the correct CSS selector for the category divs:
        # Select all div elements with class "QqxB5Ed9A73reHbF2fQC"
        category_divs = soup.select('div.QqxB5Ed9A73reHbF2fQC')

        # Extract category names
        categories = [div.text.lower() for div in category_divs]

        # Check if there are any categories, and set a default "national" category
        if not categories:
            categories = ['national']
